...
import xmltodict, json, argparse, re, os, datetime
...
class NoInputFilesError(exception):
    def __init__(self, msg):
        self.msg = "No input files provided."
    def __str__(self):
        return self.msg
...
def handle_elements(path, element):
    ...
    if type(element) is None or element is None: print('\nNo data in xml element!!\n')
    
    # ---- Add id attribute of streamed entry to element dict
    elif None not in path[1]:
        if 'id' in path[1][1].keys(): element[ s(collection_choice) + '_id' ] = int(path[1][1]['id'])
    elif 'id' in element.keys(): element[ s(collection_choice) + '_id' ] = int(element.pop('id'))
    ...
    keys_to_drop = RELEASE_DROP_KEYS if collection_choice == 'releases' else DROP_KEYS
    ...
    try:
        ...
        counter += 1
        try:
            with open(inserts_log_file_path,'a+') as log_f:
                log_f.write( str(element[collection_choice+'_id']) +'\n' )
        except FileIOError:
            print('\nCould not write to log file\n')
    except:
        duplicate_keys += 1
        try:
            with open(mongo_write_errors_log_file_path,'a+') as log_f:
                log_f.write( str(element[collection_choice+'_id']) +'\n' )
        except FileIOError:
            print('\nCould not write to log file\n')
...
if __name__ == '__main__':
    ...
    if len(args.inputfiles) < 1: raise NoInputFilesError
    
    # set up initial global variables
    global verbose_bool, mongo_indexing, inserts_log_file_path, mongo_write_errors_log_file_path
    verbose_bool,mongo_indexing = args.verbose, args.indexing
    inserts_log_file_path = '/logging/inserts_run_' + datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")+'.txt'
    mongo_write_errors_log_file_path = '/logging/mongo_errors_inserts_run' + datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")+'.txt'
